<template>
  <div class="container-fluid">
    <h2 class="text-dark m-4">Pedidos</h2>
...
export default {
  ...
  methods: {
    onClickEdit($idy) {
      const ediId = $idy;
      window.location.href = "/pedidos/editar/" + ediId + "/";
      //this.$emit("edit", ediId);
    },
    onClickDetails($idy) {
      const ediId = $idy;
      window.location.href = "/pedidos/detalle/" + ediId + "/";
      ...
      if (this.search == "") {
        ...
        this.content.forEach((value, index) => {
          if (this.foundIt(value.id, this.search)) {
            this.aux.push(value);
          } else if (this.foundIt(value.employee.name, this.search)) {
            this.aux.push(value);
          } else if (this.foundIt(value.supplier.name, this.search)) {
            this.aux.push(value);
    ...
    sort($op) {
      ...
      if ($op == 1) {
        ...
        if (this.asc) {
          this.aux.sort((a, b) =>
            a.employee.name < b.employee.name
              ? -1
              : Number(a.employee.name > b.employee.name)
          );
        } else {
          this.aux.sort((a, b) =>
            a.employee.name > b.employee.name
        ...
        if (this.asc) {
          this.aux.sort((a, b) =>
            a.supplier.name < b.supplier.name
              ? -1
              : Number(a.supplier.name > b.supplier.name)
          );
        } else {
          this.aux.sort((a, b) =>
            a.supplier.name > b.supplier.name
        ...
        if (this.asc) {
          this.aux.sort((a, b) =>
            this.getDateO(a.updated_at) < this.getDateO(b.updated_at)
              ? -1
              : Number(this.getDateO(a.updated_at) > this.getDateO(b.updated_at))
          );
        } else {
          this.aux.sort((a, b) =>
            this.getDateO(a.updated_at) > this.getDateO(b.updated_at)
    ...
    onClickDelete($idc, $iddel) {
      this.$confirm("Realmente desea eliminar el pedido", "Alerta", {
          ...
          axios
            ...
            .then((response) => {
              if (response.data != 1) {
                this.showErrorNotification(
                  "Error al eliminar",
                  "El pedido se encuentra asignado"
                );
              } else {
                this.showSuccessNotification("Eliminar", "Pedido eliminado");
                var index = this.aux.findIndex((i) => i.id === $idc);
                if (index != -1) this.aux.splice(index, 1);
                axios.get("/pedidos/all").then((res) => {
                  this.content = res.data;
                });
                this.max = Math.ceil(this.aux.length / this.pagesize);
                this.update();
    ...
    add() {
      window.location.href = "/pedidos/agregar";
    },
    users() {
      window.location.href = "/pedidos/usuarios/0";
    },
  },
};
</script>
