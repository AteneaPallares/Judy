<template>
  <div class="container-fluid">
    <h2 class="text-dark m-4">Ventas</h2>
...
export default {
  ...
  methods: {
    onClickEdit($idy) {
      const ediId = $idy;
      window.location.href = "/ventas/editar/" + ediId + "/";
      //this.$emit("edit", ediId);
    },
    onClickDetails($idy) {
      const ediId = $idy;
      window.location.href = "/ventas/detalle/" + ediId + "/";
      ...
      if (this.search == "") {
        ...
        this.content.forEach((value, index) => {
          if (this.foundIt(value.id, this.search)) {
            ...
            this.aux.push(value);
          } else if (this.foundIt(value.client.name, this.search)) {
            this.aux.push(value);
        ...
        if (this.asc) {
          this.aux.sort((a, b) =>
            a.client.name < b.client.name
              ? -1
              : Number(a.client.name > b.client.name)
          );
        } else {
          this.aux.sort((a, b) =>
            a.client.name > b.client.name
    ...
    onClickDelete($idc, $iddel) {
      this.$confirm("Realmente desea eliminar la venta", "Alerta", {
          ...
          axios
            ...
            .then((response) => {
              if (response.data != 1) {
                this.showErrorNotification(
                  "Error al eliminar",
                  "La venta se encuentra asignado"
                );
              } else {
                this.showSuccessNotification("Eliminar", "Venta eliminada");
                var index = this.aux.findIndex((i) => i.id === $idc);
                if (index != -1) this.aux.splice(index, 1);
                axios.get("/ventas/all").then((res) => {
                  this.content = res.data;
                });
                this.max = Math.ceil(this.aux.length / this.pagesize);
                this.update();
    ...
    add() {
      window.location.href = "/ventas/agregar";
    },
    users() {
      window.location.href = "/ventas/usuarios/0";
    },
  },
};
</script>
