<template>
  <div class="container-fluid">
    <h2 class="text-dark m-4">Clientes</h2>
...
export default {
  ...
  methods: {
    onClickEdit($idy) {
      const ediId = $idy;
      window.location.href = "/clientes/editar/" + ediId + "/";
      //this.$emit("edit", ediId);
    },
    onClickDetails($idy) {
      const ediId = $idy;
      window.location.href = "/clientes/detalle/" + ediId + "/";
      ...
      if (this.search == "") {
        ...
        this.content.forEach((value, index) => {
          if (this.foundIt(value.name, this.search)) {
            ...
            this.foundIt(value.phone,this.search
            )
          ) {
            this.aux.push(value);
          }
    ...
    onClickDelete($idc, $iddel, $userd) {
      this.$confirm(
        "Realmente desea eliminar al cliente " + $userd.name,
        "Alerta",
        {
          ...
          axios
            ...
            .then((response) => {
              if (response.data != 1) {
                this.showErrorNotification(
                  "Error al eliminar",
                  "El cliente se encuentra asignado"
                );
              } else {
                this.showSuccessNotification("Eliminar", "Cliente eliminado");
                var index = this.aux.findIndex((i) => i.id === $idc);
                if (index != -1) this.aux.splice(index, 1);
                axios.get("/clientes/all").then((res) => {
                  this.content = res.data;
                  this.aux = this.content;
                  this.max = Math.ceil(this.content.length / this.pagesize);
                  this.actual = 0;
