<template>
  <div class="container-fluid">
    <h2 class="text-dark m-4">Empleados</h2>
            ...
            <table class="table my-0">
              ...
              <tbody>
                ...
                >
                  ...
                  <td>
                    {{ props.role != null ? props.role.name : "-" }}
                  </td>
                  <td>
                    {{ props.email }}
                  </td>
...
export default {
  ...
  methods: {
    onClickEdit($idy) {
      const ediId = $idy;
      window.location.href = "/empleados/editar/" + ediId + "/";
      //this.$emit("edit", ediId);
    },
    onClickDetails($idy) {
      const ediId = $idy;
      window.location.href = "/empleados/detalle/" + ediId + "/";
      ...
      if (this.search == "") {
        ...
        this.content.forEach((value, index) => {
          if (this.foundIt(value.name, this.search)) {
            ...
            this.foundIt(
              value.role!=null? value.role.name : null,
              this.search
            )
          ) {
            this.aux.push(value);
    ...
    sort($op) {
      ...
      if ($op == 1) {
        ...
        if (this.asc) {
          this.aux.sort((a, b) =>
            (a.role != null ? a.role.name : "") <
            (b.role != null ? b.role.name : "")
              ? -1
              : Number(
                  (a.role != null ? a.role.name : "") >
                    (b.role != null ? b.role.name : "")
